...
RUN wget https://nightly.mtasa.com/multitheftauto_linux_x64-${MTA_SERVER_VERSION}-rc-${MTA_SERVER_BUILD_NUMBER}.tar.gz -O /tmp/mtasa.tar.gz \
    ...
    && wget http://nightly.mtasa.com/files/libmysqlclient.so.16 -P ./usr/lib \
...
RUN groupadd -r mtasa && useradd --no-log-init -r -g mtasa mtasa \
    ...
    && apt-get install -y bash tar unzip libncursesw5 wget gdb \
    # Added libssl packages
    && apt-get install -y libssl1.1 libssl-dev \
    # Add fallback in case the above fails
